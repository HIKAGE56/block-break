...
window.onload = function(){
	...
	game.fps = 30;
	game.numRows = 4;    //ブロックの横列数
	game.numColumns = 9; //ブロックの縦列数
	game.preload(
			"png/left_wall.png",
			"png/right_wall.png",
			"png/upper_wall.png",
			"png/block_frames.png",
			"png/paddle.png",
			"png/ball.png",
			"png/message_frames.png");
			//"audio/rattle.mp3");
	...
	var Background = enchant.Class.create(Sprite, {
		initialize: function() {
			...
			var bgImage = new Surface(w, h),
				ctx = bgImage.context;           //短い別名を付けて呼び出しを楽に
			...
			ctx.rect(0, 0, w, h);                //四角形を描く
			ctx.closePath();                     //パスを閉じる
			ctx.fillStyle = gradient;            //グラデーションを塗りつぶしに設定
			ctx.fill();                          //塗りつぶす
			this.image = bgImage;
		}
	});
	...
	var Wall = enchant.Class.create(Sprite, {
		...
		initialize: function(loc) {
			...
			this.x = left;
			this.y = top;
			this.image = game.assets["png/"+loc+"_wall.png"];
	...
	var Ball = enchant.Class.create(Sprite, {
		...
		initialize: function() {
			...
			this.image = game.assets["png/ball.png"];
	...
	var Block = enchant.Class.create(Sprite, {
		initialize: function(x, y, frame) {
			...
			this.image = game.assets["png/block_frames.png"];
			this.x = x;
			this.y = y;
			this.frame = frame
		}
	...
	var Paddle = enchant.Class.create(Sprite, {
		initialize: function() {
			...
			this.image = game.assets["png/paddle.png"];
	...
	var Message = enchant.Class.create(Sprite, {
		...
		initialize: function() {
			...
			this.image = game.assets["png/message_frames.png"];
